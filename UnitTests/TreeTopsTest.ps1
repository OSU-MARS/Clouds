...
# 561 tiles, 9900X: 12 threads -> 2.5 minutes, 25 GB DDR, 24 threads -> 1.6 minutes, 27 GB DDR, ~19 GB/s DDR bandwidth
#            9950X: 32 threads -> 1.1 minutes
Get-Crowns -Dsm "$dataPath\DSM v3" -Treetops "$dataPath\treetops\rf v2" -Crowns "$dataPath\treetops\crowns rf v2" -Vrt -Verbose
...
Merge-Treetops -Treetops "$dataPath\treetops\rf v2" -Classification "$dataPath\classification\PCA10iQ27 3800 1.8m m8n22 cubic subclass" -Crowns "$dataPath\treetops\crowns rf v2" -Merge "$dataPath\treetops\treetops merged rf v2 (transitory).gpkg" -Verbose
