...
$tile = "s04020w06690"
Get-Treetops -Dsm "$dataPath\DSM v3 beta\$tile.tif" -Treetops ([System.IO.Path]::Combine($env:USERPROFILE, "$dataPath\treetops DSM\$tile DSM radius.gpkg")) -Verbose

# CHM radius-based treetop identification
Get-Treetops -Method ChmRadius -Dsm "$dataPath\DSM v3 beta\s03450w06540.tif" -Treetops ([System.IO.Path]::Combine($env:USERPROFILE, "$dataPath\treetops CHM\s03450w06540.gpkg")) -Verbose
Get-Treetops -Method ChmRadius -Dsm "$dataPath\DSM v3 beta\*.tif" -Treetops ([System.IO.Path]::Combine($env:USERPROFILE, "$dataPath\treetops CHM")) -Verbose
...
#{
#  Get-Treetops -Method DsmRing -Dsm "$dataPath\DSM v3 beta\$tile.tif" -Treetops ([System.IO.Path]::Combine($env:USERPROFILE, "$dataPath\treetops DSM ring\$tile.gpkg")) -Verbose
#}