...
namespace Mars.Clouds.Las
{
    public class DigitalSurfaceModel : Raster<float>
    {
        ...
        public DigitalSurfaceModel(Grid<PointListZ> grid, RasterBand<float> dtmTile, float gapDistance, int layersToLog)
            : base(grid.Crs, grid.Transform, grid.XSize, grid.YSize, DigitalSurfaceModel.GetBandNames(layersToLog), RasterBand<float>.GetDefaultNoDataValue())
        {
            ...
            this.CanopyHeight = this.Bands[^2];
            this.RegionSize = this.Bands[^1];
        ...
        private static string[] GetBandNames(int layersToLog)
        {
            int bandCount = 1 + Int32.Max(2, layersToLog) + 2;
            string[] bandNames = new string[bandCount];

            bandNames[0] = "dsm";
            for (int bandIndex = 1; bandIndex < bandCount - 2; ++bandIndex)
            {
                bandNames[bandIndex] = "layer" + bandIndex; // ones based layer naming
            }
            bandNames[^2] = "chm";
            bandNames[^1] = "regionSize";
