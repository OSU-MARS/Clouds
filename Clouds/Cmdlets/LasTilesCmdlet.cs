...
namespace Mars.Clouds.Cmdlets
{
    public class LasTilesCmdlet : GdalCmdlet
    {
        ...
        protected int GetLasTileReadThreadCount(float readSpeedPerThreadInGBs, int minWorkerThreadsPerReadThread)
        {
        ...
        protected LasTileGrid ReadLasHeadersAndFormGrid(string cmdletName, int? requiredEpsg)
        {
            ...
            int readThreads = this.GetLasTileReadThreadCount(DriveCapabilities.HardDriveDefaultTransferRateInGBs, minWorkerThreadsPerReadThread: 0);

            List<LasTile> lasTiles = new(lasTilePaths.Count);
            int tileReadsInitiated = -1;
            ParallelTasks readLasHeaders = new(Int32.Min(readThreads, this.MaxThreads), () =>
            {
