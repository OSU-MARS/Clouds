...
namespace Mars.Clouds.Cmdlets
{
    ...
    public class GetDtm : LasTilesToTilesCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            const string cmdletName = "Get-Dtm";
            bool dtmPathIsDirectory = Directory.Exists(this.Dtm);
            int dtmTileSizeX = -1;
            int dtmTileSizeY = -1;
            LasTileGrid? lasGrid = null;
            List<string>? lasFilePaths = null;
            int lasFilesToRead;
            int lasFilesOrGridPositionsToRead;
            if (this.Vrt)
            {
                lasGrid = this.ReadLasHeadersAndFormGrid(cmdletName, nameof(this.Dtm), dtmPathIsDirectory);
                (this.CellSize, dtmTileSizeX, dtmTileSizeY) = LasTilesToTilesCmdlet.GetRasterSizing(lasGrid, this.CellSize);
                lasFilesToRead = lasGrid.NonNullCells;
                lasFilesOrGridPositionsToRead = lasGrid.Cells;
            }
            else
            {
                lasFilePaths = this.GetExistingFilePaths(this.Las, Constant.File.LasExtension);
                lasFilesToRead = lasFilePaths.Count;
                lasFilesOrGridPositionsToRead = lasFilePaths.Count;
            }

            TileReadWrite dtmReadWrite = new(dtmPathIsDirectory);
            ...
            int preferredCompletionThreadsWithFewTiles = Int32.Min(this.DataThreads, 25 - (int)(1.5F * lasFilesOrGridPositionsToRead));
            int tileCompletionThreads = Int32.Min(lasFilesOrGridPositionsToRead, Int32.Max(preferredCompletionThreadsWithFewTiles, preferredCompletionThreadsAsymptotic));
            ...
            ParallelTasks dtmTasks = new(readThreads + tileCompletionThreads, () =>
            {
                ...
                for (int lasFileIndex = dtmReadWrite.GetNextTileReadIndexThreadSafe(); lasFileIndex < lasFilesOrGridPositionsToRead; lasFileIndex = dtmReadWrite.GetNextTileReadIndexThreadSafe())
                {
                    ...
                    try
                    {
                        ...
                        if (this.Vrt)
                        {
                            Debug.Assert(lasGrid != null);
                            lasFile = lasGrid[lasFileIndex];
                            if (lasFile == null)
                            {
                        ...
                        {
                            Debug.Assert(lasFilePaths != null);
                            string lasFilePath = lasFilePaths[lasFileIndex];
                            using LasReader lasReader = LasReader.CreateForHeaderAndVlrRead(lasFilePath, this.DiscardOverrunningVlrs);
                            lasFile = new(lasFilePath, lasReader, fallbackCreationDate: null);
                        ...
                        reader = lasFile.CreatePointReader(this.Unbuffered, enableAsync: false);
                        reader.ReadGroundPointsToDtm(lasFile, dtmTile, ref pointReadBuffer);
                    }
                    finally
                    {
                        reader?.Dispose();
                    ...
                    dtmReadWrite.IncrementTilesReadThreadSafe();
            ...
            TimedProgressRecord progress = new(cmdletName, dtmReadWrite.GetLasReadTileWriteStatusDescription(lasFilesToRead, activeReadThreads, dtmTasks.Count));
            ...
            while (dtmTasks.WaitAll(Constant.DefaultProgressInterval) == false)
            {
                ...
                progress.StatusDescription = dtmReadWrite.GetLasReadTileWriteStatusDescription(lasFilesToRead, activeReadThreads, dtmTasks.Count);
                progress.Update(dtmReadWrite.TilesWritten, lasFilesToRead);
                this.WriteProgress(progress);
            }
            ...
            this.WriteVerbose($"{cellsWritten:n0} DTM cells in {dtmReadWrite.TilesRead} point cloud {(dtmReadWrite.TilesRead == 1 ? "tile" : "tiles")} in {progress.Stopwatch.ToElapsedString()}: {dtmReadWrite.TilesWritten / progress.Stopwatch.Elapsed.TotalSeconds:0.0} tiles/s.");
            base.ProcessRecord();
        }
    }
}
