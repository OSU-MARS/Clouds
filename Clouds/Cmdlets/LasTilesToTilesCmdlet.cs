...
namespace Mars.Clouds.Cmdlets
{
    public abstract class LasTilesToTilesCmdlet : LasTilesCmdlet
    {
        ...
        protected static (double cellSize, int xSizeInCells, int ySizeInCells) GetRasterSizing(SpatialReference crs, double specifiedCellSize, double rasterWidthInCrsUnits, double rasterHeightInCrsUnits)
        {
            ...
            else if (specifiedCellSize <= 0.0)
            {
                throw new ArgumentOutOfRangeException(nameof(specifiedCellSize), $"Since raster cell sizes must be positive the specified cell size of {specifiedCellSize} is not valid.");
            }
        ...
        protected static (double cellSize, int xSizeInCells, int ySizeInCells) GetTileRasterSizing(LasTileGrid lasGrid, double specifiedCellSize)
        {
